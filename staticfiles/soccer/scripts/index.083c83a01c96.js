...
function convertToLocalTime(dateString) {
    // Try to parse the date string as ISO 8601
    let parsedDate = new Date(dateString);

    // Check if the date is invalid (NaN)
    if (isNaN(parsedDate.getTime())) {
        // If the date is not ISO, try adding 'T00:00:00Z' to the string
        parsedDate = new Date(dateString + 'T00:00:00Z');
    }

    // If parsing still fails, return 'Invalid Date'
    if (isNaN(parsedDate.getTime())) {
        return 'Invalid Date';
    }

    // Convert to local time based on the user's time zone
